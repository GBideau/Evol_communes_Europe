...
Ce script présente la mise en graphique de données issues de sources secondaires. Le but est d'avoir une vision synthétique de l'évolution du nombre de communes (ou entités équivalentes) dans plusieurs pays européens.
...
Il est possible d'accéder au code de ce Markdown [ici](https://gbideau.github.io/Evol_communes_Europe/evol_Europe.Rmd) et au fichier regroupant les données (intégrant les métadonnées) [là](https://gbideau.github.io/Evol_communes_Europe/niveau_communal_europe.xlsx).
...
# Librairies (pas nécessairement toutes utilisées)
library(sf)
library(cartography)
library(mapsf)
library(corrplot)
...
Liste des pays membres de l'UE ou limitrophes de la France
Source des données 1950, 2007 et Appellation1 : Comité européen sur la démocratie locale et régionale (CDLR) et Nogueira López Alba, 2007, Les relations entre les autorités centrales et les collectivités locales, s.l., Conseil de l’Europe. Quand plusieurs niveaux étaient disponibles, on a choisi ceux pour lesquels des données étaient disponibles dans les autres sources.
https://rm.coe.int/16807480b3 

NB : Les données "1950" sont parfois à une autre date, cf. source initiale.

Source des données 2001 et Appellation2 : Motte Claude et Vouloir Marie-Christine, 2008, « Frontières administratives et identités communales. Le cas de la France, XVIII-XXe siècles », The Historical Review/La Revue Historique, 2008, vol. 5, p. 213‑238.


Source des données 2012, 2016 et Appellation3 : Conseil des Communes et Régions d’Europe, 2016, Gouvernements locaux et régionaux en Europe. Structures et compétences, s.l., CCRE. 
